...
>Tip) 금광의 모든 위치에 대하여 다음의 세 가지만 고려하면 된다.
> 1. 왼쪽 위에서 오는 경우
> 2. 왼쪽 아래에서 오는 경우
> 3. 왼쪽에서 오는 경우
...
const n = 3;
const m = 4;
const arr = [1, 3, 3, 2, 2, 1, 4, 1, 0, 6, 4, 7];

const dp = [];
let cnt = 0;
while(dp.length !== n) {
  dp.push(arr.slice(cnt, cnt+m));
  cnt += 4;
}
...
for(let j = 1; j < m; j++) {
  for(let i = 0; i < n; i++) {
    ...
    left = dp[i][j-1];
    dp[i][j] = dp[i][j] + Math.max(left_up, left_down, left);
  }
}
...
>Tip) 가장 긴 증가하는 부분 수열(LIS)로 알려진 전형적인 다이나믹 프로그래밍 문제의 아이디어다.
> 모든 0 <= j < i에 대하여, 
> d[i] = max(d[i], d[i]+1) if array[j] < array[i]
